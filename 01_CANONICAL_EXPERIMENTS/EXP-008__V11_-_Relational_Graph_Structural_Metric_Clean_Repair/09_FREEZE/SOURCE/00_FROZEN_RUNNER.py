...
EXPERIMENT = "V6_CLEAN_FINGERPRINT_CONFIRMATORY_REPLICATION"

MODEL = "gpt-5.6-luna"

OUT = Path(
    r"C:\RSOS\RSOS  Python Results\V6_CLEAN_FINGERPRINT_CONFIRMATORY_REPLICATION"
)
...
# =====================================================================
# CLEAN CONFIRMATORY RESPONSE VALIDITY
# =====================================================================

def extract_valid_visible_text(response):
...
def execute(seed, condition, rep):
    ...
    for attempt in range(1, 9):
        ...
        try:
            ...
            text = extract_valid_visible_text(
                response
            )

            if text is None:

                usage_invalid = getattr(
                    response,
                    "usage",
                    None
                )
                ...
                raise RuntimeError(
                    ...
                    f"|output_tokens={invalid_output_tokens}"
