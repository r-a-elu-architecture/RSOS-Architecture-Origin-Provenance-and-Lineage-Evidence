...
EXPERIMENT = "V12_RELATIONAL_TOPOLOGY_CAUSAL_MAP_CLEAN"
MODEL = "gpt-5.6-luna"
MASTER_SEED = 120918
SEED_IDS = list(range(1000, 1080))
N_REPS = 3
DEV = set(range(1000, 1040))
CONF = set(range(1040, 1060))
HOLD = set(range(1060, 1080))
MAX_OUTPUT_TOKENS = 500
MAX_RETRIES = 8
N_PERMUTATIONS = 20000
ROOT = Path(r"C:\RSOS")
SOURCE_ROOT = ROOT / "V11_STRUCTURAL_METRIC_REPAIR"
SOURCE_MANIFEST = SOURCE_ROOT / "SOURCE_SHA256_MANIFEST.csv"
V11_RESULT_ROOT = ROOT / "RSOS  Python Results" / "V11_OPENAI_RELATIONAL_GRAPH_FINGERPRINT_FINAL"
OUT = ROOT / "RSOS  Python Results" / EXPERIMENT
...
NATIVE_FRAMES = {
    "N1_COHERENT":
        "Choose a coherent nontrivial directed dependency structure among the supplied opaque nodes.",

    "N2_STABLE":
        "Choose a stable internally consistent directed relational structure among the supplied opaque nodes.",

    "N3_COMPACT":
        "Choose a compact nontrivial directed interaction structure among the supplied opaque nodes."
}
...
def topology_vector(edges):
    ...
    sccsizes = sorted(
        [
            len(x)
            for x
            in nx.strongly_connected_components(g)
        ],
        reverse=True
    ) + [0] * 8
...
AUTH_COMPLEXITY = complexity_vector(
    AUTH_EDGES
)

AUTH_TOPOLOGY = topology_vector(
    AUTH_EDGES
)
...
def degree_preserving_rewire(seed):
    ...
    raise RuntimeError(
        f"degree rewire failed seed={seed}"
    )
...
INSTRUCTION = (
    ...
    "S specifies a preferred relation transformation; when S names an active old edge and an absent new edge, use it jointly when forming the next state. "
...
def validate_output(
    ...
    active_edges = active_label_edges(
        payload
    )
...
def native_prompt(
    ...
    inst = (
        ...
        "Do not use named theories, architectures, domains, hierarchies, feedback terminology, or semantic labels."
    )
...
def verify_sources():
    ...
    pd.DataFrame(
        [
            {
                "SOURCE":
                    "SELF_CONTAINED_V12_CLEAN",

                "STATUS":
                    "NO_EXTERNAL_HASH_COMPARISON",
...
def run_analysis(
    ...
    for part, seeds in [
        ...
        for family, conds in [
            (
                "DELETE",
                EDGE_DELETE_CONDITIONS
            ),
            (
                "REVERSE",
                EDGE_REVERSE_CONDITIONS
            )
        ]:
            local = []
...
def analyze_existing():
    ...
    successful = load_unique(
        raw
    )

    nsuccess = load_unique(
        nraw
    )
...
def main():
    ...
    if args.preflight_only:
        print(
            "V12 CLEAN zero-cost preflight starting...",
            flush=True
        )
    ...
    if args.self_test:
        print(
            "V12 CLEAN full downstream analysis self-test...",
            flush=True
        )
    ...
    nerr = (
        OUT
        /
        "26_NATIVE_ERRORS.jsonl"
    )

    napi = (
        OUT
        /
        "26_NATIVE_RAW_API_OBJECTS.jsonl.gz"
    )
